...
actual class NetworkScanner actual constructor() {
    ...
    actual fun scanNetwork(port: Int, timeoutMs: Int): Flow<DiscoveredDevice> = flow {
        ...
        PlatformLogger.d("NetworkScanner", "Scanning subnet: $subnet.* for port $port")

        // Scan IPs 1-254 in the subnet
        for (i in 1..254) {
            if (isCancelled.get() || !coroutineContext.isActive) {
                PlatformLogger.d("NetworkScanner", "Scan cancelled")
                break
            }
            ...
            try {
                ...
                PlatformLogger.d("NetworkScanner", "Found device at $ip:$port")
        ...
        PlatformLogger.d("NetworkScanner", "Scan complete")
    ...
    actual suspend fun getLocalIpAddress(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            while (interfaces.hasMoreElements()) {
                ...
                while (addresses.hasMoreElements()) {
                    ...
                    if (address is Inet4Address && !address.isLoopbackAddress) {
                        ...
                        // Prefer WiFi (wlan) or typical private network addresses
                        if (ip != null && (ip.startsWith("192.168.") || ip.startsWith("10.") || ip.startsWith("172."))) {
                            PlatformLogger.d("NetworkScanner", "Local IP: $ip")
            ...
            PlatformLogger.e("NetworkScanner", "Failed to get local IP", e)
        }
        null
    }
